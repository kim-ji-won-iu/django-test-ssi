...
def upload_location(instance, filename, **kwargs):
    file_path = 'about/{author_id}/{title}-{filename}'.format(
...
@receiver(post_delete, sender=AboutPost)
def submission_delete(sender, instance, **kwargs):
    instance.image.delete(False)


def pre_save_about_post_receiever(sender, instance, *args, **kwargs):
    if not instance.slug:
        instance.slug = unique_slug_generator(instance)
        # instance.slug = slugify(instance.author.username + "-" + instance.title)


pre_save.connect(pre_save_about_post_receiever, sender=AboutPost)
